...
- **C'est au moment où tu clôtures l'US** (GO NEXT après revue OK à l'étape TDD-front-end → étape = `done`) qu'il est pertinent de lancer la suite de tests (et le build) pour valider avant de considérer l'US terminée.
...
**Dernière mise à jour** : 2026-01-30 (2 commandes : GO US, GO NEXT ; revue → pause → GO NEXT suivant = corrections / agent suivant / clôture)
