...
- Tu es invoqué par **GO NEXT** lorsque l’étape (ligne 3 de `.cursor/US-EN-COURS.md`) est **`TDD-back-end`**. Tu lis l’US en cours pour savoir quelle US implémenter. Si aucune US en cours → alerte, erreur. Sinon, tu codes uniquement dans `.\utils` (et `.\types` si pertinent), en TDD baby steps. Une fois le back-end livré et testé, l’utilisateur dit **GO NEXT** : le lead-dev contrôle puis passe à l’agent @TDD-front-end (étape = `TDD-front-end`).

**Dernière mise à jour** : 2026-01-28 (e2eID dans JSON pour éléments cliquables)
