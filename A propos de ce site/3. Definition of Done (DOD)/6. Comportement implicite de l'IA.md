...
##### 3bis. Collecte automatique des métriques
**Règle** : Lors de chaque build :
1. Lancer automatiquement `npm run metrics:collect` pour collecter les métriques
...
##### 3ter. Couverture de code à 100%
**Règle** : Exigence de qualité stricte :
1. La couverture de code doit être maintenue à **100%** sur tous les critères (lignes, statements, functions, branches)
2. Chaque nouveau code doit avoir ses tests unitaires AVANT d'être validé (TDD strict)
3. Si la couverture descend en dessous de 100%, corriger immédiatement
4. Exécuter `npm test -- --coverage` régulièrement pour vérifier la couverture
...
3. Pour les règles TDD/BDD, suivre strictement :
   - **Un test à la fois** (le plus simple possible)
   - **RED** : Écrire un seul test qui échoue
   - **GREEN** : Écrire le code minimal pour faire passer le test
   - **REFACTOR** : Améliorer le code (nommage métier/DDD, lisibilité)
...
4. **Étape 4 - Code** : Coder seulement après validation de toutes les étapes précédentes, en suivant **TDD strict** :
   - **Un test à la fois** (commencer par le plus simple)
   - **Code minimal** (juste assez pour faire passer le test)
   - **Refactoriser après chaque vert** (avant de passer au test suivant)
