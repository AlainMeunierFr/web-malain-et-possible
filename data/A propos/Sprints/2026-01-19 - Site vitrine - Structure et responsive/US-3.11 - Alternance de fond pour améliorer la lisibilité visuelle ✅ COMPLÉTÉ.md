...
## CA1 - Alternance automatique : Les composants `DomaineDeCompetences` alternent automatiquement entre fond blanc (index impair) et fond bleu clair (index pair)
## CA2 - Remise à zéro sur les titres : Chaque titre (`Titre` composant) remet à zéro le compteur d'alternance, garantissant que le premier domaine après un titre est toujours blanc
## CA3 - Vidéos exclues : Les composants `Video` ont toujours un fond blanc et ne participent pas à l'alternance
## CA4 - Gestion dynamique : L'alternance est calculée dynamiquement dans `PageContentRenderer` en fonction de la position dans la liste, évitant les problèmes lors de la réorganisation des JSON
## CA5 - Styles CSS
...
## CA7 - Note : Cette US sera traitée après US-3.10a, une fois la structure de données validée