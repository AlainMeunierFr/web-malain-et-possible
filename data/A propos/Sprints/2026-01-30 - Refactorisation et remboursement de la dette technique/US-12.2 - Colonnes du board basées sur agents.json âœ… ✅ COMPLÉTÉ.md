...
## En tant que visiteur ou membre du projet qui consulte le tableau de bord « A propos » (board KanBan du sprint)
...
## CA1 - CA1 - Source des colonnes
- La liste des colonnes « agents » du board KanBan est **toujours** construite à partir du fichier `data/A propos/agents.json` (et non plus à partir de la lecture directe du dossier `.cursor/agents`).
...
## CA3 - CA3 - Synchronisation en dev
- En environnement de développement, un mécanisme (script ou étape de build/dev) crée ou met à jour `data/A propos/agents.json` à partir des noms des fichiers du dossier `.cursor/agents` (fichiers `.md`, ordre dérivé du préfixe numérique ou du nom de fichier).
