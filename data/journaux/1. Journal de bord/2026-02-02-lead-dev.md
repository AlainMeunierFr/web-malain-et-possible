...
#### Nettoyage et rangement des fichiers MD

##### Prompt
Audit des fichiers MD suspects avec arbitrages utilisateur

##### Résultat technique
**13 fichiers supprimés** (~58 Ko) :
- `LISTE-COMPETENCES-DOMAINES-NON-UTILISES.md`
- `liste-domaines-competences-temp.md`
- `LISTE-VIDEOS-PAR-PROFIL.md`
- `PROPOSITION-DOMAINES-PAR-PROFIL.md`
- `docs/effets-de-bord-raw-mode.md`
- `analyse-resultat.txt`, `Refacto.txt`, `styles-texte.txt`
- `test-error.txt`, `test-full.txt`
- `debug-us31-json.json`, `comparaison-arbitrage.json`, `competences-domaines-non-utilises.json`

**4 fichiers déplacés** :
- `HIERARCHIE-SITE-ASCII.md` → `docs/generated/`
- `RAPPORT-REFACTO-NOMS-CANONIQUES.md` → `docs/generated/`
- `prompt-designer-mise-en-page.md` → `.cursor/`
- `proposition-mise-en-page-design.md` → `.cursor/`

**Impact** : +1 sur Documentation (projet plus propre)

---

### Génération de la base de connaissance

#### Lecture complète des journaux et génération de 3 documents de synthèse

##### Prompt
A impatient de voir le rapport ;-) (et de le corriger... ou pas)
Un objectif supplémentaire en lisant les journaux et de m'aider à mieux formuler mes prompts (pour être plus efficace et consommer moins de token)

##### Résultat technique
Lecture de **67 fichiers** de journaux (Architecture, Questions/Arbitrages, Journal de bord, Cours) et génération de **3 documents de synthèse** :

**1. `docs/KNOWLEDGE-BASE.md`** (~4 Ko)
Base de connaissance complète du projet incluant :
- Vue d'ensemble (stack, objectif, expérience parallèle)
- Architecture (hexagonale, parsing MD unifié, CMS JSON)
- Méthodologie (workflow Lead Dev, TDD strict, tests d'intégration qui modifient le code)
- Système de métriques (collecte, historique, tendances)
- Stratégie E2E (couverture liens, plages réservées e2eID)
- Décisions architecturales majeures (nommage canonique, containers implicites, JSON-LD)
- Points de vigilance (tests préexistants, path traversal, sécurité)
- Agents disponibles et bonnes pratiques

**2. `docs/PROBLEMES-RESOLUS.md`** (~6 Ko)
Catalogue de **15 problèmes majeurs** avec causes racines et solutions :
- Bataille des tooltips (React Portal)
- Confusion "dessous" vertical vs z-index
- Tests désynchronisés après refactoring
- E2eID conflictuels (plages réservées)
- Métriques hardcodées ("NC")
- Chronométrage BDD/E2E mélangé
- Plan du site vide sur Vercel
- Path traversal dans les lecteurs de fichiers
- Et 7 autres problèmes documentés

**3. `docs/GUIDE-PROMPTS-EFFICACES.md`** (~5 Ko)
Guide pratique basé sur l'analyse des patterns efficaces et des pièges :
- Règle d'or : Action + Contexte + Critères + Exemple
- 4 patterns efficaces avec exemples
- 5 pièges à éviter (ambiguïté terminologique, scope implicite, etc.)
- Formats recommandés (correction technique, nouvelle fonctionnalité, bug)
- Commandes rapides (GO US, GO NEXT, A, B, 0, C puis A)
- Checklist avant d'envoyer un prompt
- 6 conseils pour économiser des tokens
- 3 règles d'or : Spécificité > Généralité, Contexte > Assumptions, Structure > Flux

**Impact** : Documentation projet enrichie, base de connaissance exploitable pour l'onboarding et la maintenance
