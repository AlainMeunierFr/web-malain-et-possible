...
- `app/(main)/charte/` → `app/(main)/a-propos/charte/`
- Nouvelle route : `/a-propos/charte`
...
- Entrée ajoutée dans `data/A propos de ce site/menu.json` :
  ...
  ```
- `MenuAPropos.tsx` gère le cas `parametre === "charte"` → `/a-propos/charte`

##### 3. Neutralisation CSS
Le CSS `a-propos.css` contenait déjà des règles `:has(.charte)` pour neutraliser les surcharges et conserver le rendu vitrine.
...
### Board Kanban : affichage du dernier sprint quand tout est terminé

#### Problème
La page "Sprint en cours" n'affichait rien quand le fichier `US en cours.md` était vide (toutes les US terminées).

#### Solution
Modification de l'API `/api/sprint-board` :
1. **Fallback sur le dernier sprint** : si aucune US en cours, affiche le sprint le plus récent (tri par date du nom de dossier)
2. **Carte fictive humoristique** : US-9000.0 "Prendre un café bien mérité ☕" en état "Review" (loupe 🔍)

#### Nouvelle fonction
`getLatestSprintFolder()` dans `sprintBoardReader.ts` : retourne le chemin du sprint le plus récent.

#### Contenu de l'US fictive
Critères d'acceptation basés sur la DOD :
- TDD respecté (RED → GREEN → REFACTOR)
- Couverture > 80%
- Pas de code sans test
- Principes SOLID appliqués
- US validées avant dev
- Scénarios BDD en français
- Revue de code à chaque étape
- Journal de bord à jour

#### Tests ajoutés
3 tests pour `getLatestSprintFolder` :
- Retourne le dossier le plus récent
