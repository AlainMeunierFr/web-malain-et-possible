...
describe('Test d\'intégration : Validation des fichiers MD réels', () => {
  ...
  it('devrait valider la syntaxe des User Stories dans User Stories.md', () => {
    // ARRANGE : Chemin vers le fichier User Stories.md
    const userStoriesPath = path.join(process.cwd(), 'A propos de ce site', '1. A propos du projet', 'User Stories.md');
    
    // Vérifier que le fichier existe
    if (!fs.existsSync(userStoriesPath)) {
      throw new Error(`Le fichier User Stories.md n'existe pas : ${userStoriesPath}`);
    }

    // ACT : Lire et parser le fichier
    const contenu = fs.readFileSync(userStoriesPath, 'utf-8');
    const result = parseSectionContent(contenu);
    
    // ASSERT : Vérifier que toutes les User Stories ont les 4 éléments requis
    const erreurs: string[] = [];
    
    for (const partie of result.parties) {
      for (const sousPartie of partie.sousParties) {
        // Vérifier si c'est une User Story (H4)
        // Une User Story doit avoir les 4 typeDeContenu
        const typeDeContenus = sousPartie.contenuParse
          .filter(e => e.typeDeContenu)
          .map(e => e.typeDeContenu);
        
        const estUserStory = typeDeContenus.length > 0;
        
        if (estUserStory) {
          const typeDeContenusUniques = new Set(typeDeContenus);
          const typesRequis = ['En tant que', 'Je souhaite', 'Afin de', 'Critères d\'acceptation'];
          
          for (const typeRequis of typesRequis) {
            if (!typeDeContenusUniques.has(typeRequis)) {
              erreurs.push(
                `User Story "${sousPartie.titre}" dans la partie "${partie.titre}" manque l'élément "${typeRequis}"`
              );
            }
          }
        }
    ...
    // ASSERT : Aucune erreur ne doit être trouvée
    if (erreurs.length > 0) {
      const messageErreur = `Erreurs de syntaxe User Stories trouvées :\n${erreurs.join('\n')}`;
      throw new Error(messageErreur);
    }
  });
});
