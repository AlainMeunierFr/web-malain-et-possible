...
import type { PlanLien } from './siteMapGenerator';
import type { E2eIdInventoryItem } from './e2eIdInventory';
import { generateE2eIdFromUrl } from './e2eIdFromUrl';
...
/**
 * Génère le contenu complet du fichier spec E2E Playwright (même format qu'actuellement).
 * @param chemin Liste ordonnée des URLs des pages visitées (étapes du scénario)
 * @param liens Liens du plan (pour labels et détection lien direct)
 * @param pages Pages du plan (url, titre) pour les commentaires
 * @param inventory Inventaire des e2eID (footer, header, json, react)
 */
    ...
    if (i === 0) {
      ...
      }
      {
        ...
        if (hasDirectLink && label && label !== '') {
          ...
          lignes.push(`      } else {`);
          const errEscaped = `Impossible de trouver un lien vers ${page} depuis ${pagePrecedente} (label: "${escapedLabelRegex}").`.replace(/'/g, "\\'");
          ...
          if (page === '/metrics') {
            ...
            lignes.push(`      } else {`);
            const errMsg = `Impossible de trouver le bouton Plan du site (e2eid-b13) dans le footer.`.replace(/'/g, "\\'");
            lignes.push(`        throw new Error('${errMsg}');`);
            lignes.push(`      }`);
          }
          lignes.push(`    }`);
        } else {
          if (page === '/metrics') {
            ...
            lignes.push(`      await lienDepuisPlan${i}.first().click();`);
            lignes.push(`    } else {`);
            const errMsg2 = `Impossible de trouver le bouton Plan du site (e2eid-b13) dans le footer.`.replace(/'/g, "\\'");
            lignes.push(`      throw new Error('${errMsg2}');`);
            lignes.push(`    }`);
          }
        }
      }
